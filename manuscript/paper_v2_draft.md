...
6. **No aggregation clearance or ATP feedback terms**: The current model has no decay term on aggregation and no ATP feedback on aggregation growth (`d_agg` is ATP-independent by design; `simulator.py` lines 97–100). Clearance-enhancing interventions (autophagy inducers, proteasome activators) and ATP-dependent chaperone activity cannot be modelled. Adding explicit clearance kinetics or bidirectional ATP–aggregation coupling would enable a third intervention class in future therapeutic-mapping phases, and may change the apparent dominance of ISR suppression. A planned v3.0 will implement ATP-dependent chaperone kinetics (`−CHAPERONE_RATE × atp × agg`); until then, ATP restoration experiments (post-R4 phases) reflect model architecture rather than biological dynamics and should not be cited as mechanistic findings.
